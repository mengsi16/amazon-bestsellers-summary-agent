...
| **写** 报告 | `{workspace}/reports/<category_slug>_aplus_dim.md` | Markdown 报告 |
| **写** JSON | `{workspace}/reports/<category_slug>_aplus_dim.json` | JSON 摘要 |
| **写** 图片 | `{workspace}/reports/aplus_images/` | A+ 图片下载 |
...
└──────────┬────────────────┘
           ...
           ▼
┌───────────────────────────┐
│ Step 3: 下载 Top5 图片     │  → 运行 fetch_aplus_images.py 爬虫
└──────────┬────────────────┘
           │
           ▼
┌───────────────────────────┐
...
### Step 3: 下载 Top5 A+ 图片（推荐 plan，支持降级）

**在分析前，必须先完成以下两步：**

#### Step 3a：你（模型）读取数据源并构造下载计划

1. 读取 `{workspace}/chunks/global_manifest.json` 定位 Top5 产品目录
2. 对每个 Top5 产品，读取 `aplus/extract/aplus_extracted.md`，提取 `## Image Assets` 表格中所有 URL
3. 若上述无结果，读取 `aplus/raw/aplus.html`，用正则匹配 `data-src=` 和 `src=` 中的 `aplus-media` URL
4. 将所有 URL 汇居，写入下载计划文件：

```json
// 写入 {workspace}/reports/aplus_images/download_plan.json
{
    ...
    "products": [
        {
            ...
            "urls": [
                "https://m.media-amazon.com/images/S/aplus-media/img1.jpg",
                "https://m.media-amazon.com/images/S/aplus-media/img2.jpg"
            ]
        }
    ]
}
...
若 plan 文件不可用或格式校验失败，使用降级模式直接传 URL：
...
工具产出：
- 图片文件：`{workspace}/reports/aplus_images/<dir_name>/aplus_img_001.jpg` 等
- 下载清单：`{workspace}/reports/aplus_images/download_manifest.json`

> ⚠️ **fail-fast**：如果 `download_plan.json` 缺少必填字段（`output_dir`、`products`、`dir_name`、`urls`），工具会立即退出并找出错误，修复后重运行。

> ⚠️ **真实执行校验**：只有当 `{workspace}/reports/aplus_images/<dir_name>/` 下存在真实图片文件且 `download_manifest.json` 为脚本产出结构时，才允许进入视觉分析。
...
4. **图片先行**：对 Top5 产品，必须先运行 `fetch_aplus_images.py` 下载图片再进行视觉分析。
...
- [ ] 已读取 Top5 的 `aplus_extracted.md` / `aplus.html` 提取图片 URL
- [ ] 已写入 `{workspace}/reports/aplus_images/download_plan.json`
- [ ] 已调用 `fetch_aplus_images.py --download-plan` 完成下载
- [ ] 若 plan 模式失败，已改用 `--output-dir + --product` 降级模式重试
- [ ] `download_manifest.json` 已生成
- [ ] `{workspace}/reports/aplus_images/` 下存在真实图片文件（非仅 plan/manifest）
