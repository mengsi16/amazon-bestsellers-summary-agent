...
14. **禁止中断流程询问用户**：流水线一旦启动，必须自动完整执行到输出 `summary.md` 为止。**禁止在任何步骤停下来问用户**"要不要开始""下一步是否继续""请确认""是否继续执行""已完成 Step X，是否进行 Step Y"等。遇到任何错误（爬虫失败、部分 ASIN 缺失、子 agent 报错）一律记录后继续推进，不阻塞流水线，不等待用户回复。`summary.md` 是唯一的终止条件。
