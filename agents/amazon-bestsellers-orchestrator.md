...
| `https://www.amazon.com/gp/bestsellers/electronics/172282` | `172282` |
...
Step 2b: crawl_product_details → 爬取商品详情页（ASIN 去重；自动提取 listing + A+ 图）
    ...
    │
    ▼
Step 3: amazon-product-chunker agent → 分块 + 提取（4 个块：ppd/reviews/details/aplus）
        读取 ← {workspace}/products/{ASIN}/product.html
             + {workspace}/categories/{browse_node_id}/rankings.jsonl（取排名）
        写入 → {workspace}/chunks/{rank}_{ASIN}/
    │
    ▼
...
1. **提取 browse_node_id（category_slug）**：
   - 用户必须提供 Amazon Bestsellers **URL**，如 `https://www.amazon.com/gp/bestsellers/fashion/1040658/`
   - 从 URL 路径中抽取尾部数字串作为 `browse_node_id` = `category_slug`（本例 = `1040658`）
   - **禁止模型自己起名**（不要 `womens-hoodies`、`fashion-top50` 之类）
   - 如果用户只给了名称没给 URL，**必须先问用户要完整的 Bestsellers URL**

2. **创建 workspace 目录**：
   ```
   mkdir -p {CWD}/workspace/{browse_node_id}/products
   mkdir -p {CWD}/workspace/{browse_node_id}/categories
   mkdir -p {CWD}/workspace/{browse_node_id}/chunks
   mkdir -p {CWD}/workspace/{browse_node_id}/reports
   ```
   （实际上 scraper MCP 会自动创建这些子目录，你只需确保 `{workspace}` 根目录存在即可。）
...
4. **顺序执行**：scraper → chunker → analysts → summary，不可跳步。
